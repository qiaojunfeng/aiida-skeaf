...
* [`aiida_skeaf/`](aiida_skeaf/): The main source code of the plugin package
  * [`data/`](aiida_skeaf/data/): A new `DiffParameters` data class, used as input to the `DiffCalculation` `CalcJob` class
  * [`calculations.py`](aiida_skeaf/calculations.py): A new `DiffCalculation` `CalcJob` class
  * [`cli.py`](aiida_skeaf/cli.py): Extensions of the `verdi data` command line interface for the `DiffParameters` class
  * [`helpers.py`](aiida_skeaf/helpers.py): Helpers for setting up an AiiDA code for `diff` automatically
  * [`parsers.py`](aiida_skeaf/parsers.py): A new `Parser` for the `DiffCalculation`
...
```shell
verdi data skeaf list
verdi data skeaf export <PK>
```
